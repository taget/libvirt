...
static int virResCtrlReadConfig(int type)
{
    int ret;
    size_t i, nbanks;
    char *str;
    ...
    resctrlall[type].cache_banks = virHostCPUGetCacheBanks(type,
                                                           resctrlall[type].cbm_len, &nbanks);
...
virResCtrlInit(void)
{
    ...
    for (i = 0; i < VIR_RDT_RESOURCE_LAST; i++) {
        ...
        if (virFileExists(tmp)) {
            if ((rc = virResCtrlReadConfig(i)) < 0) {
                virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
