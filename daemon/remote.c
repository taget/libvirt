...
static int
remoteDispatchDomainGetCacheTune(virNetServerPtr server ATTRIBUTE_UNUSED,
                                 virNetServerClientPtr client ATTRIBUTE_UNUSED,
                                 virNetMessagePtr msg ATTRIBUTE_UNUSED,
                                 virNetMessageErrorPtr rerr,
                                 ...
                                 remote_domain_get_cache_tune_ret *ret)
{
    ...
    if (virDomainGetCacheTune(dom, &params, &nparams, args->flags) < 0)
        goto cleanup;

    if (nparams > REMOTE_NODE_CACHE_STATS_MAX) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("nparams too large"));
        goto cleanup;
    }
